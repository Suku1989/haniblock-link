    ...
    listen (port, host) {
        ...
        this._httpServer.on('error', e => {
            this.isSameServer('127.0.0.1', this._port).then(isSame => {
                if (isSame) {
                    console.log(`Port is already used by other haniblock-link server, will try reopening after ${REOPEN_INTERVAL} ms`); // eslint-disable-line max-len
                    setTimeout(() => {
        ...
        this._httpServer.listen(this._port, '0.0.0.0', () => {
            this.emit('ready');
            console.info(clc.green(`haniblock link server start successfully, socket listen on: https://${this._host}:${this._port}`));
        });
    }
}
